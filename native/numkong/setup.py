...
def linux_settings() -> tuple[list[str], list[str], list[tuple[str, str]]]:
    ...
    compile_args = [
        ...
        "-fPIC",
        "-w",  # Hush warnings
        "-fopenmp",  # OpenMP for ColBandit parallel doc scoring
    ]
...
def darwin_settings() -> tuple[list[str], list[str], list[tuple[str, str]]]:
    ...
    # OpenMP on macOS: Apple Clang does not ship with OpenMP runtime.
    # Requires: brew install libomp
    # Detect libomp prefix from Homebrew; fall back gracefully if not installed.
    try:
        ...
        # libomp not installed: OpenMP disabled. colbandit.c parallel loops fall back to serial.
        compile_args.append("-DNK_NO_OPENMP=1")
        print("[NumKong] WARNING: libomp not found (brew install libomp). OpenMP disabled.")
