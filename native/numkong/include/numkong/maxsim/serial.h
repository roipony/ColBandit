...
NK_INTERNAL void nk_maxsim_quantize_vector_(                             //
    ...
    // Pass 1: Find top-6 absolute values + compute norm_squared
    // p95 clipping: use 6th largest |value| as scale instead of max.
    // Equalizes quantization scales across vectors with outliers.
    // Zero runtime cost (6-element insertion sort vs single max tracking).
    nk_f32_t norm_squared_f32 = 0.0f;
    nk_f32_t top6[6] = {0, 0, 0, 0, 0, 0};
    for (nk_size_t dim_index = 0; dim_index < depth; dim_index++) {
        ...
        if (abs_value > top6[5]) {
            ...
            for (int k = 4; k >= 0; k--) {
                if (top6[k+1] > top6[k]) {
                    nk_f32_t tmp = top6[k]; top6[k] = top6[k+1]; top6[k+1] = tmp;
                } else break;
            }
        }
    }
    // Use 6th largest as scale; values beyond clip to ±scale_limit
    nk_f32_t clip_val = top6[5] > 0.0f ? top6[5] : top6[0];
    nk_f32_t scale_f32 = clip_val / scale_limit;
...
/**
 *  @brief Per-token MaxSim: returns per-query-token angular distances instead of sum.
 *  Output: result[query_index] = angular distance for each query token.
 *  Used by Col-Bandit for Serfling bound computation (needs per-token statistics).
 */
NK_PUBLIC void nk_maxsim_packed_per_token_f32_serial( //
    void const *query_packed, void const *document_packed, nk_size_t query_count, nk_size_t document_count,
    nk_size_t depth, nk_f64_t *result) {
    ...
    for (nk_size_t chunk_start = 0; chunk_start < query_count; chunk_start += 256) {
        ...
        for (nk_size_t query_index = 0; query_index < chunk_size; query_index++) {
            ...
            nk_f64_t angular = 1.0 - cosine;
            if (angular < 0.0) angular = 0.0;
            result[chunk_start + query_index] = angular;  /* store per-token, don't sum */
        }
    }
}

/**
 *  @brief MaxSim with Welford stats (serial fallback): returns (sum, sum_sq) in result[0..1].
 */
NK_PUBLIC void nk_maxsim_packed_stats_f32_serial( //
    void const *query_packed, void const *document_packed, nk_size_t query_count, nk_size_t document_count,
    nk_size_t depth, nk_f64_t *result) {
