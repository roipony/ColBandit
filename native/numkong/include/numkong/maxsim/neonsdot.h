...
/**
 *  @brief MaxSim with Welford stats for NEONSDOT: returns (sum, sum_sq) in result[0..1].
 *  Coarse i8 argmax via vdotq_s32 (signed×signed) to find best candidate per query token,
 *  then f32 dot refinement. Accumulates sum and sum-of-squares for Serfling variance estimates
 *  used by CB-NK warmup-phase elimination.
 */
NK_PUBLIC void nk_maxsim_packed_stats_f32_neonsdot( //
    void const *query_packed, void const *document_packed, nk_size_t query_count, nk_size_t document_count,
    nk_size_t depth, nk_f64_t *result) {
